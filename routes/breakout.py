...
def _score_ticker(ticker, is_bull=True, rvol_min=1.5):
    ...
    if dec >= 3:
        ...
        s += 5
    if avg20 > 0 and volumes[-1] / avg20 < 0.5:
        s += 5
    s = min(s, 15)
    if dec >= 3:
    ...
    s = 0
    ex = 'Insufficient data for BB squeeze'
    if len(closes) >= 40:
        def _bbw(c20):
            sm = sum(c20) / 20
        ...
        if hist_w:
            ...
            if pct < 0.40:
                s = 15
                ex = f'BB width at {round(pct * 100)}% of avg — extreme squeeze, breakout imminent'
            elif pct < 0.60:
                s = 10
                ex = f'BB width at {round(pct * 100)}% of avg — tight squeeze building'
            elif pct < 0.75:
                s = 5
                ex = f'BB width at {round(pct * 100)}% of avg — bands narrowing'
            else:
                ex = f'BB width at {round(pct * 100)}% of avg — no meaningful squeeze'
    ...
    if rsi is None:
        ...
        s = 10
        ex = f'RSI {rsi} — ideal pre-breakout zone (45-65)'
    elif (35 <= rsi < 45) or (65 < rsi <= 75):
        s = 5
        ex = f'RSI {rsi} — acceptable but not ideal launch zone'
    elif rsi > 75:
    ...
    if hl >= 3:
        s = 10
        ex = f'{hl} consecutive higher lows — buyers stepping in at higher prices'
    elif hl >= 2:
        s = 5
        ex = f'{hl} consecutive higher lows — early demand structure forming'
    else:
        ex = 'No higher lows pattern — structure not yet established'
    ...
    if len(closes) >= 11:
        ...
        if obv_chg > 0.01 and abs(p_chg10) < 0.02:
            s = 20
            ex = 'OBV rising while price flat — stealth accumulation, smart money loading'
        elif obv_chg > 0.01:
            s = 10
            ex = 'OBV rising with price — volume confirming the uptrend'
        else:
    ...
    if pct_from_high <= 1:
        s = 20
        ex = f'Within {pct_from_high}% of 52-wk high — at breakout resistance'
    elif pct_from_high <= 3:
        s = 15
        ex = f'{pct_from_high}% from 52-wk high — approaching key resistance'
    elif pct_from_high <= 5:
        s = 10
        ex = f'{pct_from_high}% from 52-wk high — within striking distance'
    elif pct_from_high <= 10:
        s = 5
        ex = f'{pct_from_high}% from 52-wk high — building base below resistance'
    else:
        s = 0
        ex = f'{pct_from_high}% from 52-wk high — too far from breakout level'
    sigs.append({'name': '52-Wk High Proximity', 'score': s, 'max': 20, 'pass': s >= 10, 'explanation': ex})

    # ── 8. Pocket Pivot (0-20) ────────────────────────────────────────────
    s = 0
    ex = 'Insufficient data for pocket pivot'
    if len(closes) >= 12:
        ...
        if pp_vol and pp_pos:
            s = 20
            ex = 'Volume exceeds all down-day vols and close in upper range — textbook pocket pivot'
        elif pp_vol:
            s = 10
            ex = 'Volume clears down-day highs but close in lower half of range'
        elif pp_pos:
            s = 10
            ex = 'Close in upper range but volume below down-day threshold'
        else:
    ...
    if trump_mention:
        sigs.append({'name': '🔴 Trump Mentioned Today', 'score': 20, 'max': 20, 'pass': True,
                     'explanation': 'Trump publicly mentioned this ticker today — retail flood typically follows within hours'})  # noqa: E501
...
# ── Weekly Breakout (UW-powered) ──────────────────────────────────────────────

_uw_scan_cache = {}
UW_SCAN_TTL = 1800  # 30 min


@breakout_bp.route('/weekly-breakout')
def weekly_breakout_page():
    return render_template('weekly_breakout.html', active='breakout')


@breakout_bp.route('/api/breakout/uw-scan')
def uw_scan():
    ...
    now = datetime.now()
    cache_key = f'uw_{preset}'
    cached = _uw_scan_cache.get(cache_key)
    if cached and (now - cached['ts']).total_seconds() < UW_SCAN_TTL:
        return jsonify(cached['data'])

    from utils.unusual_whales import uw_breakout_screener
    from concurrent.futures import ThreadPoolExecutor

    try:
        uw_items = uw_breakout_screener(preset)
    except Exception:
        uw_items = []
    ...
    with ThreadPoolExecutor(max_workers=6) as ex:
        scored = list(ex.map(_score, tickers))
