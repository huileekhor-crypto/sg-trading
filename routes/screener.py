...
Be specific, direct, no fluff."""
        ...
        return jsonify({
            ...
            "pre_market":    live.get('pre_market'),
            "post_market":   live.get('post_market'),
            "sgt_time":      live.get('sgt_time'),
...
def _beat_rate(history):
    ...
    if not valid:
        return None, 0
    beats = sum(1 for a, ev in valid if a >= ev)
...
@screener_bp.route('/earnings/calendar', methods=['GET'])
def get_earnings_calendar():
    from flask import session as flask_session
    user_id  = str(flask_session.get('user_id', 'guest'))
    now      = datetime.now()
    ...
    try:
        ...
        user_wl = list(dict.fromkeys(user_wl + watchlist_store.get(user_id, {}).get('user', [])))
    except Exception:
        pass

    all_tickers = set(user_wl + _TOP_MOVERS)
    cache_key   = f"{user_id}_{','.join(sorted(user_wl))}"
    cached      = _calendar_cache_e.get(cache_key)
    ...
    try:
        ...
        for item in relevant:
            ...
            beat_rate, beat_cnt = _beat_rate(hist)
            beat_color          = ('neutral' if beat_rate is None else
                                   'green'   if beat_rate >= 60 else
                                   'red'     if beat_rate <= 40 else 'amber')
        ...
        result = {
            'events': events, 'user_watchlist': user_wl,
            'date_from': date_from, 'date_to': date_to,
            'timestamp': now.isoformat(),
        }
        _calendar_cache_e[cache_key] = {'data': result, 'ts': now}
        return jsonify(result)
...
@screener_bp.route('/earnings/ticker/<ticker>', methods=['GET'])
def get_earnings_ticker(ticker):
    ticker = ticker.upper().strip()
    now    = datetime.now()

    cached = _ticker_cache_e.get(ticker)
    if cached and (now - cached['ts']).total_seconds() < 21600:
        return jsonify(cached['data'])
    ...
    try:
        ...
        for e in history[:8]:
            ...
            if actual is not None and estimate is not None:
                beat = ('beat' if actual > estimate * 1.01 else
                        'miss' if actual < estimate * 0.99 else 'inline')

            quarters.append({
                'period': date_str, 'quarter': e.get('quarter'), 'year': e.get('year'),
                'actual': actual,   'estimate': estimate,
                'surprise_pct': round(float(e.get('surprisePercent') or 0), 2),
                'beat': beat, 'price_move': price_move,
            })
        ...
        result = {'ticker': ticker, 'quarters': quarters,
                  'beat_rate': beat_rate, 'avg_move': avg_move}
        _ticker_cache_e[ticker] = {'data': result, 'ts': now}
        return jsonify(result)
